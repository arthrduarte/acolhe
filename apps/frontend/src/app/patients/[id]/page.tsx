...
export default function PatientProfile() {
  ...
  const [patient] = useState({
    ...
    reviews: [
      ...
      { id:2, stars: 4, comment: "Great patient, always punctual and cooperative." }
    ]
  })
    ...
    <div className='container mx-auto flex flex-col gap-8 py-12 px-4 max-w-4xl'>
      <div className='flex gap-8 flex-col md:flex-row items-center md:items-start'>
        ...
        <div className='flex-1 text-center md:text-left space-y-3'>
          ...
          <p className='text-lg text-muted-foreground'>{patient.age} years old, {patient.gender}</p>
          ...
          <p className='text-muted-foreground flex items-center justify-center md:justify-start gap-2'>
            ...
            {match === "pending" ? "Phone available after match" : patient.phone}
          </p>
        </div>
      </div>

      <div className='bg-white rounded-xl shadow-sm p-6 space-y-4'>
        <h2 className='text-2xl font-bold text-gray-800'>About {patient.name.split(" ")[0]}</h2>
        <p className='text-gray-600 leading-relaxed'>{patient.about}</p>
      </div>
    </div>
  )
}